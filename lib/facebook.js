...
var debug = require('sherlock')('fbagent')
...
/**
 * setup suparagent-proxy handler
 *
 * this will add an operation '.proxy(server)' to the request object
 */
...
Facebook.prototype.send = function (opts) {
  this.postData = extend(this.postData || {}, opts || {});
  return this;
};
...
Facebook.prototype.end = function (cb) {
  ...
  if (this.method === 'GET') {
    ...
    if (acc) request.query({ 'access_token': acc });
    ...
    if (acc) request.send('access_token=' + acc);
  } else {
